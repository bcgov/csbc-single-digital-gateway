...
import { BcscAuthController } from 'src/modules/auth/controllers/bcsc-auth.controller';
...
describe('BCSCAuthController - Integration Tests', () => {
  let app: INestApplication;
  let server: http.Server;
  let controller: BcscAuthController;
  const loginEndpoint = '/auth/bcsc/login';
  const callbackEndpoint = '/auth/bcsc/callback';
  const logoutEndpoint = '/auth/bcsc/logout';
  const meEndpoint = '/auth/bcsc/me';
  ...
  beforeAll(async () => {
    const module = await Test.createTestingModule({
      controllers: [BcscAuthController],
      providers: [
    ...
    controller = module.get<BcscAuthController>(BcscAuthController);
  });
  ...
  describe("@Get('login')", () => {
    ...
    it('Should use FRONTEND_URL when returnTo is not provided in query', async () => {
      mockAuthService.buildAuthorizationUrl.mockResolvedValue(
        buildAuthorizationUrl,
      );
    ...
    it('Should handle empty returnTo query parameter and use FRONTEND_URL', async () => {
      mockAuthService.buildAuthorizationUrl.mockResolvedValue(
        buildAuthorizationUrl,
      );
    ...
    it('Should handle whitespace-only returnTo and use FRONTEND_URL', async () => {
      mockAuthService.buildAuthorizationUrl.mockResolvedValue(
        buildAuthorizationUrl,
      );
  ...
  describe('@Post("logout")', () => {
    ...
    it('should clear session.bcsc so /me becomes unauthorized after logout', async () => {
      const agent = request.agent(server);
  ...
  describe("@Get('me')", () => {
    ...
    it('should call authService.getUserProfile with IdpType.BCSC and session', async () => {
