...
import { ServiceTypesAdminV1Controller } from '../service-types-admin-v1.controller';

describe('ServiceTypesAdminV1Controller', () => {
  let controller: ServiceTypesAdminV1Controller;
  ...
  beforeEach(() => {
    ...
    controller = new ServiceTypesAdminV1Controller(
  ...
  it('findAll isAdmin should be false when no userId', async () => {
    typesService.findAll.mockResolvedValue({ data: [] });
    await controller.findAll(
      { page: 1, limit: 10 } as never,
  ...
  it('findAllPublished should delegate', () => {
    controller.findAllPublished({ page: 1, limit: 10, search: 'test' } as never);
    expect(typesService.findAllPublished).toHaveBeenCalledWith(1, 10, 'test');
  });
  ...
  it('upsertTranslation should delegate', () => {
    ...
    controller.upsertTranslation(
      { typeId: 't1', versionId: 'v1', locale: 'en' } as never,
      body as never,
    );
