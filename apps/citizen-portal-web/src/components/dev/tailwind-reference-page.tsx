...
const graySwatches: ColorSwatchItem[] = [
  ...
  },
  {
    ...
    bg: 'bg-gray-110',
    text: 'text-gray-110',
    border: 'border-gray-110',
    cssVar: '--color-gray-110',
  },
];

const semanticSurfaceExamples: SemanticSurfaceItem[] = [
  { label: 'bg-background text-foreground', className: 'bg-background text-foreground' },
  { label: 'bg-card text-card-foreground', className: 'bg-card text-card-foreground' },
  { label: 'bg-popover text-popover-foreground', className: 'bg-popover text-popover-foreground' },
  { label: 'bg-primary text-primary-foreground', className: 'bg-primary text-primary-foreground' },
  {
    label: 'bg-secondary text-secondary-foreground',
    className: 'bg-secondary text-secondary-foreground',
  },
  { label: 'bg-accent text-accent-foreground', className: 'bg-accent text-accent-foreground' },
  { label: 'bg-muted text-muted-foreground', className: 'bg-muted text-muted-foreground' },
  {
...
const statusTokenGroups: StatusTokenGroup[] = [
  ...
  },
  {
    ...
    // NOTE: theme.css names this token --color-danger-surface-text (not --color-danger-text like
    // the other three), so the generated utility is text-danger-surface-text. Using the real
    // current name rather than the old site's text-danger-text so this reference stays accurate.
    textClass: 'text-danger-surface-text',
  },
];

const SECTION_ICONS: Record<string, string> = {
  'color-palette': mdiPaletteSwatch,
  'background-and-semantic-color-helpers': mdiPalette,
  typography: mdiFormatFont,
  'spacing-borders-radius-and-shadows': mdiRuler,
  structure: mdiViewDashboard,
  groupings: mdiViewGrid,
  'coming-next': mdiCreation,
};

export function TailwindReferencePage() {
  const contentRef = useRef<HTMLDivElement>(null);
  const navItems = useDevPageNav(contentRef, SECTION_ICONS);

  return (
    <DevPageLayout
      title="Tailwind Helper Class Reference"
      description="A practical one-pager showing common helpers in this app, alongside rendered examples and copyable patterns."
      navItems={navItems}
    ...
    >
      ...
      <DevSection
        id="background-and-semantic-color-helpers"
      ...
      >
        <div className="grid gap-8 lg:grid-cols-2">
          <div className="space-y-4">
            <h3 id="core-semantic-surfaces">Core semantic surfaces</h3>
            <div className="grid gap-3">
              {semanticSurfaceExamples.map((item) => (
                ...
                >
                  <code className="text-sm font-semibold text-bcgov-blue">{item.label}</code>
                  <div className={`rounded-md border border-border px-4 py-3 ${item.className}`}>
                    Example surface
                  </div>
                </div>
              ))}
      ...
      >
        <div className="space-y-8">
          <div className="space-y-4">
            <h3 id="heading-elements-vs-utility-classes">Heading elements vs utility classes</h3>
            <div className="grid gap-4">
              <HeadingComparisonRow
                label="<h1>"
                elementPreview={<h1>Heading level 1</h1>}
                classLabel="text-h1 font-bold"
                classPreview={<div className="text-h1 font-bold">Utility heading 1</div>}
              />
              <HeadingComparisonRow
                label="<h2>"
                elementPreview={<h2>Heading level 2</h2>}
                classLabel="text-h2 font-bold"
                classPreview={<div className="text-h2 font-bold">Utility heading 2</div>}
              />
              <HeadingComparisonRow
                label="<h3>"
                elementPreview={<h3>Heading level 3</h3>}
                classLabel="text-h3 font-bold"
                classPreview={<div className="text-h3 font-bold">Utility heading 3</div>}
              />
              <HeadingComparisonRow
                label="<h4>"
                elementPreview={<h4>Heading level 4</h4>}
                classLabel="text-h4 font-bold"
                classPreview={<div className="text-h4 font-bold">Utility heading 4</div>}
              />
              <HeadingComparisonRow
                label="<h5>"
                ...
                classPreview={
                  <div className="text-h5 font-bold">Utility heading 5 (same as H4)</div>
                }
              />
              <HeadingComparisonRow
                label="<h6>"
                ...
                classPreview={
                  <div className="text-lg font-bold">Utility heading 6 (same as H4)</div>
                }
              />
            </div>
          </div>
          ...
          <div className="space-y-4">
            <h3 id="common-text-helpers">Common text helpers</h3>
            <div className="grid gap-4">
              <TypeRow label="text-sm" preview={<div className="text-sm">Small body text</div>} />
              <TypeRow
                label="text-base"
                preview={<div className="text-base">Base body text</div>}
              />
              <TypeRow label="text-lg" preview={<div className="text-lg">Large body text</div>} />
              <TypeRow
                label="text-xl"
              ...
              <TypeRow
                label="text-2xl"
                preview={<div className="text-2xl">2x Extra large body text</div>}
              />
              <TypeRow
                label="text-display"
              ...
              <TypeRow
                ...
                preview={<div className="font-semibold">Semibold helper (renders bold)</div>}
              />
              <TypeRow label="font-bold" preview={<div className="font-bold">Bold weight</div>} />
              <TypeRow
                label="text-link underline"
                preview={
                  <div className="text-link underline underline-offset-3">Link-styled text</div>
                }
              />
              <TypeRow
                label="text-muted-foreground"
                preview={<div className="text-muted-foreground">Muted text</div>}
              />
              <TypeRow
                label="text-disabled"
      ...
      <DevSection
        id="spacing-borders-radius-and-shadows"
        ...
        <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              All the usual Tailwind spacing helpers work off the same 4px multiplier:{' '}
              <code>p-*</code>, <code>px-*</code>, <code>py-*</code>, <code>m-*</code>,{' '}
              <code>gap-*</code>, and <code>space-*</code>.
            </p>
              ...
              <SpacingDemoCard
                ...
                preview={
                  <div className="space-y-4 rounded-md border border-border bg-background p-4">
                    <div className="rounded border border-border bg-card px-3 py-2">Item 1</div>
                    <div className="rounded border border-border bg-card px-3 py-2">Item 2</div>
                    <div className="rounded border border-border bg-card px-3 py-2">Item 3</div>
                  </div>
                }
              />
            </div>
          ...
          <div className="space-y-4">
            <h3 id="foundation-helpers">Foundation helpers</h3>
            ...
            <MiniPreviewRow
              ...
              preview={
                <div className="flex flex-wrap gap-3">
                  <div className="rounded-md border bg-card px-3 py-2">1px</div>
                  <div className="rounded-md border-2 border-border bg-card px-3 py-2">2px</div>
                  <div className="rounded-md border-4 border-border bg-card px-3 py-2">4px</div>
                </div>
              }
            />
            ...
            <MiniPreviewRow
              ...
              preview={
                <div className="flex flex-wrap gap-3">
                  <div className="rounded-none border border-border bg-card px-3 py-2">none</div>
                  <div className="rounded-sm border border-border bg-card px-3 py-2">sm</div>
                  <div className="rounded-md border border-border bg-card px-3 py-2">md</div>
                  <div className="rounded-lg border border-border bg-card px-3 py-2">lg</div>
                  <div className="rounded-full border border-border bg-card px-3 py-2">full</div>
                </div>
              }
            />
  ...
  <section>Section 3</section>
              ...
              preview={
                <div className="space-y-4 rounded-md border border-border bg-background p-4">
                  <div className="rounded-md border border-border bg-card px-4 py-3">Section 1</div>
                  <div className="rounded-md border border-border bg-card px-4 py-3">Section 2</div>
                  <div className="rounded-md border border-border bg-card px-4 py-3">Section 3</div>
                </div>
              }
            />

            <CodeExampleCard
              id="basic-two-column-layout"
	...
	<div class="grid gap-px">
            ...
            >
              <div className="flex flex-col gap-px border bg-border">
                <div className="grid gap-px">
                  <div className="bg-white p-4">
                    <p>Single column, but with grey border to match above.</p>{' '}
                  </div>
                </div>
              </div>
            </ComponentDemoCard>
      ...
      >
        <div className="grid gap-4 md:grid-cols-3">
          <Link
            to="/dev/button"
            className="rounded-lg border border-border bg-background p-6 text-center no-underline hover:bg-blue-10 transition-colors"
          >
            <div className="font-semibold text-foreground">Button</div>
            <div className="mt-1 text-sm text-muted-foreground">Variants, sizes & links</div>
          </Link>
          <Link
            to="/dev/accordion"
            className="rounded-lg border border-border bg-background p-6 text-center no-underline hover:bg-blue-10 transition-colors"
          >
            <div className="font-semibold text-foreground">Accordion</div>
            <div className="mt-1 text-sm text-muted-foreground">Accordion & AccordionGroup</div>
          </Link>
          <TodoCard title="Layouts" />
        </div>
      </DevSection>
    </DevPageLayout>
  );
...
function ColorPaletteRow({ name, swatches }: { name: string; swatches: ColorSwatchItem[] }) {
  return (
    <div className="space-y-3">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        {swatches.map((swatch) => (
          <ColorSwatch key={`${name}-${swatch.step}`} swatch={swatch} name={name} />
        ))}
      </div>
    </div>
  );
...
function ColorSwatch({ swatch, name }: { swatch: ColorSwatchItem; name: string }) {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
      ...
      <div className="space-y-1 p-4">
        ...
        <code className="block text-xs text-muted-foreground">{swatch.text}</code>
        <code className="block text-xs text-muted-foreground">{swatch.border}</code>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="grid gap-4 border-b border-border pb-4 last:border-b-0 last:pb-0 lg:grid-cols-[110px_minmax(0,1fr)_minmax(0,1fr)] lg:items-center">
      ...
      <div className="space-y-2 rounded-md border border-border bg-background p-4">
        <div className="text-xs text-muted-foreground">Rendered heading element</div>
        {elementPreview}
      </div>

      <div className="space-y-2 rounded-md border border-border bg-background p-4">
        <code className="block text-xs text-muted-foreground">{classLabel}</code>
        {classPreview}
      </div>
    </div>
  );
...
function TypeRow({ label, preview, note }: { label: string; preview: ReactNode; note?: string }) {
  return (
