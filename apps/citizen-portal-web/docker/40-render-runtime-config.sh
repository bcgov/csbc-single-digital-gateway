...
# API_UPSTREAM is the host:port nginx reverse-proxies /api/* to (the citizen-portal-api Service); it
# is substituted into the nginx config by 20-envsubst-on-templates.sh. Guard it for a clear error.
: "${API_UPSTREAM:?API_UPSTREAM must be set — the BFF Service nginx proxies /api/* to, e.g. citizen-portal-api:4000}"
