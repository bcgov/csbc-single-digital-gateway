...
/**
 * Mount two Swagger UIs split on the `/v1` path prefix: the base (unversioned: `/auth`, `/health`) at
...
export function setupSwagger(app: INestApplication, nodeEnv: string): void {
  ...
  const config = new DocumentBuilder()
    .setTitle('Citizen Portal API')
    .setDescription('Citizen-facing BFF for the Single Digital Gateway.')
    .setVersion('1.0')
    .build();
