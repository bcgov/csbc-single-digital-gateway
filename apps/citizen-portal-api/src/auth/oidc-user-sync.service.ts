...
/** The `user_role` enum element type, derived from the schema (no drift). */
type UserRole = NonNullable<(typeof users.$inferInsert)['roles']>[number];
...
export class OidcUserSyncService implements AuthUserSync {
  ...
  async onSignIn(claims: OidcClaims): Promise<AuthUser> {
    ...
    // Baseline role for a NEW user (this app's audience); re-login never changes existing roles.
    const defaultRole = this.config.getOrThrow<UserRole>('AUTH_DEFAULT_ROLE');
    ...
    const userId = await this.db.transaction(async (tx: DbTx) => {
      ...
      // First login: create the canonical user with this app's baseline role + identity.
