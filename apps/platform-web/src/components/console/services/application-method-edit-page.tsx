...
import { type MultiStageDefinition, emptyDefinition } from '@/components/stage-builder/stage-model';
import {
  type FormDefinition,
...
/** Multi-stage method editor — seeded from the loaded form, with an editable form title. */
function StageEdit({ form }: { form: FormWithVersion }) {
  const shell = useShellProps();
  const [docTitle, setDocTitle] = useState(form.form.title);
  const seeded = form.version.schema as unknown as MultiStageDefinition;
  const [value, setValue] = useState<MultiStageDefinition>(
    seeded.stages ? seeded : emptyDefinition(),
  );
  const queryClient = useQueryClient();
  const save = useMutation({
    mutationFn: () =>
      updateFormSchema(form.form.id, form.version.id, {
        definition: value,
        title: docTitle.trim() || form.form.title,
      }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['forms', 'detail', form.form.id] }),
  });
  return (
    <ApplicationShell
      {...shell}
      label={docTitle || form.form.title}
      onSave={() => save.mutate()}
      saving={save.isPending}
      error={save.error}
      titleSlot={
        <>
          <Label htmlFor="method-form-title" className="sr-only">
            Form title
          </Label>
          <Input
            id="method-form-title"
