...
import { AddServiceTypeTranslationDialog } from "../../../../../../../../features/admin/service-types/components/add-service-type-translation-dialog.component";
import { ServiceTypeVersionTranslationForm } from "../../../../../../../../features/admin/service-types/components/service-type-version-translation-form.component";
import {
  useArchiveServiceTypeVersion,
  usePublishServiceTypeVersion,
} from "../../../../../../../../features/admin/service-types/data/service-types.mutations";
...
export const Route = createFileRoute(
  ...
  loader: async ({ params }) => {
    await queryClient.ensureQueryData(
      serviceTypeVersionQueryOptions(params.typeId, params.versionId),
    );
  },
  staticData: {
    breadcrumbs: () => [
  ...
  component: ServiceTypeVersionDetailPage,
});

function ServiceTypeVersionDetailPage() {
  const { typeId, versionId } = Route.useParams();
  const [showPublish, setShowPublish] = useState(false);
  const [showArchive, setShowArchive] = useState(false);

  const { data: version, isLoading, error } = useQuery(
    serviceTypeVersionQueryOptions(typeId, versionId),
  );

  const publishMutation = usePublishServiceTypeVersion(typeId);
  const archiveMutation = useArchiveServiceTypeVersion(typeId);
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          {isDraft && (
            <AddServiceTypeTranslationDialog
              typeId={typeId}
              versionId={versionId}
