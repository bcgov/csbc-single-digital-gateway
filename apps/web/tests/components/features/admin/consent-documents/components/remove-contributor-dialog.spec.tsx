...
import type { Contributor } from "src/features/admin/consent-documents/data/consent-documents.query";
...
import { RemoveContributorDialog } from "src/features/admin/consent-documents/components/remove-contributor-dialog.component";

const CONTRIBUTOR_WITH_NAME: Contributor = {
  userId: "aaaa-1111-aaaa-1111",
  name: "Robert Brown",
  email: "robert.brown@gov.bc.ca",
  role: "owner",
...
describe("RemoveContributorDialog", () => {
  afterEach(cleanup);
  ...
  it("displays contributor name in the confirmation message with document context", () => {
    render(
      <RemoveContributorDialog
        contributor={CONTRIBUTOR_WITH_NAME}
        isPending={false}
    ...
    expect(screen.getByText(/from this document/)).toBeInTheDocument();
  });
  ...
  it("falls back to email when contributor name is null", () => {
    ...
    expect(screen.getByText(/noname@gov\.bc\.ca/)).toBeInTheDocument();
  });
  ...
  it("renders Cancel and Remove buttons as enabled when not pending", () => {
    render(
      <RemoveContributorDialog
        contributor={CONTRIBUTOR_WITH_NAME}
        isPending={false}
