...
import { CreateDocumentTypeDialog } from "src/features/admin/consent-document-types/components/create-document-type-dialog.component";

describe("CreateDocumentTypeDialog", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockIsPending = false;
  });
  ...
  it("shows the dialog heading and JsonForms stub when opened", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Create Document Type (JSON Forms)" }),
    ).toBeInTheDocument();
  ...
  it("shows the descriptive helper text inside the dialog", () => {
    ...
    expect(
      screen.getByText(/This will create a new document type/),
    ).toBeInTheDocument();
  });

  it("renders the Create Document Type submit button as enabled", () => {
    renderDialog();
    openDialog();

    const submitBtn = screen.getByRole("dialog").querySelector("button[type='submit']")!;
    expect(submitBtn).toBeInTheDocument();
    expect(submitBtn).toHaveTextContent("Create Document Type");
