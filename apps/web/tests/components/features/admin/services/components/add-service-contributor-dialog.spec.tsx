...
import { AddServiceContributorDialog } from "src/features/admin/services/components/add-service-contributor-dialog.component";

describe("AddServiceContributorDialog", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockIsPending = false;
  });
  ...
  it("does not call mutate when the user ID field is empty on submit", () => {
    renderDialog();
    openDialog();
  ...
  it("calls mutate with trimmed userId and role 'owner' on valid submit", () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("Enter user ID"), {
      target: { value: "  user-abc-456  " },
    });
    fireEvent.submit(screen.getByRole("dialog").querySelector("form")!);

    expect(mockMutate).toHaveBeenCalledTimes(1);
    expect(mockMutate).toHaveBeenCalledWith(
      { userId: "user-abc-456", role: "owner" },
