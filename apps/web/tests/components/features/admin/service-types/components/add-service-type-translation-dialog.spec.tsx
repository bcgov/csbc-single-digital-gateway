...
import { AddServiceTypeTranslationDialog } from "src/features/admin/service-types/components/add-service-type-translation-dialog.component";

describe("AddServiceTypeTranslationDialog", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("Continue button is enabled for a new locale", () => {
    renderDialog(["en"]);
    openDialog();
  ...
  it("advances to translation form with correct locale after clicking Continue", () => {
    ...
    expect(screen.getByTestId("translation-form")).toHaveAttribute("data-locale", "ko");
    expect(screen.queryByPlaceholderText("en")).not.toBeInTheDocument();
  });

  it("stays on locale step if Continue is clicked with empty input", () => {
    renderDialog();
    openDialog();
  ...
  it("trims whitespace from locale before advancing", () => {
    ...
    expect(screen.getByTestId("translation-form")).toHaveAttribute("data-locale", "nl");
  });
});
