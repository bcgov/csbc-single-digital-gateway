...
jest.mock("src/features/admin/service-types/components/create-service-type-dialog.component", () => ({
  CreateServiceTypeDialog: ({ trigger, onCreated }: any) => (
    <div data-testid="create-dialog">
      {trigger}
      <button onClick={() => onCreated({ id: "new-type-1" })}>TriggerCreated</button>
    </div>
  ),
}));
jest.mock("src/features/admin/service-types/components/delete-service-type-dialog.component", () => ({
  DeleteServiceTypeDialog: ({ typeId, onConfirm, onCancel }: any) => (
...
jest.mock("src/features/admin/service-types/components/service-types-table.component", () => ({
  ServiceTypesTable: ({ types, onPageChange, onDelete }: any) => (
    <div data-testid="types-table" data-count={types?.length ?? 0}>
      <button onClick={() => onPageChange(2)}>NextPage</button>
      <button onClick={() => onDelete("type-to-delete")}>DeleteRow</button>
    </div>
  ),
}));
jest.mock("src/features/admin/service-types/data/service-types.mutations", () => ({
  useDeleteServiceType: () => ({ mutate: mockDeleteMutate, isPending: false }),
}));
jest.mock("src/features/admin/service-types/data/service-types.query", () => ({ serviceTypesQueryOptions: jest.fn() }));
...
import { Route } from "src/app/routes/admin/settings/services/service-types/index";
...
describe("Admin Service Types Index Route", () => {
  beforeEach(() => {
  ...
  it("Should register route", () => {
    expect((createFileRoute as unknown as jest.Mock)).toHaveBeenCalledWith("/admin/settings/services/service-types/");
  });

  it("Should return breadcrumbs", () => {
    expect(typedRoute.options.staticData.breadcrumbs()).toEqual([
      { label: "Settings", to: "/admin/settings" }, { label: "Service Types" },
    ]);
  });
  ...
  describe("Interaction: goToPage", () => {
    it("Should call navigate with page 2 when ServiceTypesTable fires onPageChange(2)", () => {
      mockUseQuery.mockReturnValue({ data: { data: [{ id: "t1" }], page: 1, totalPages: 3 }, isLoading: false, error: null });
      render(<typedRoute.options.component />);
    ...
    it("Should set page to undefined when navigating to page 1", () => {
      mockUseQuery.mockReturnValue({ data: { data: [{ id: "t1" }], page: 2, totalPages: 3 }, isLoading: false, error: null });
      jest.requireMock("src/features/admin/service-types/components/service-types-table.component").ServiceTypesTable = ({ types, onPageChange, onDelete }: any) => (
        <div data-testid="types-table" data-count={types?.length ?? 0}>
  ...
  describe("Interaction: setDeletingTypeId via onDelete", () => {
    it("Should set typeId on DeleteServiceTypeDialog when a row delete is triggered", () => {
      // Restore to original interactive mock
      jest.requireMock("src/features/admin/service-types/components/service-types-table.component").ServiceTypesTable = ({ types, onPageChange, onDelete }: any) => (
        <div data-testid="types-table" data-count={types?.length ?? 0}>
          <button onClick={() => onPageChange(2)}>NextPage</button>
          <button onClick={() => onDelete("type-to-delete")}>DeleteRow</button>
      ...
      expect(screen.getByTestId("delete-dialog")).toHaveAttribute("data-type-id", "type-to-delete");
    });
    ...
    it("Should call deleteMutation.mutate with the correct typeId when ConfirmDelete is clicked", () => {
      ...
      expect(mockDeleteMutate).toHaveBeenCalledWith("type-to-delete", expect.any(Object));
    });
    ...
    it("Should show toast.success after successful delete and clear deletingTypeId", () => {
      ...
      fireEvent.click(screen.getByText("DeleteRow"));
      fireEvent.click(screen.getByText("ConfirmDelete"));
      expect(toast.success).toHaveBeenCalledWith("Service type deleted.");
      expect(screen.getByTestId("delete-dialog")).toHaveAttribute("data-type-id", "none");
    });
  });

  describe("Interaction: handleCreated navigates to new service type", () => {
    it("Should navigate to the new type page when CreateServiceTypeDialog fires onCreated", () => {
      mockUseQuery.mockReturnValue({ data: null, isLoading: false, error: null });
      render(<typedRoute.options.component />);
