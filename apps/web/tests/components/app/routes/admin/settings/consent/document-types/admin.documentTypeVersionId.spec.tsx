...
jest.mock(
  "src/features/admin/consent-document-types/components/add-type-translation-dialog.component",
  () => ({
    AddTypeTranslationDialog: ({ typeId, versionId, trigger }: any) => (
      <div
...
jest.mock(
  ...
  () => ({
    useArchiveTypeVersion: () => ({ mutate: jest.fn(), isPending: false }),
    usePublishTypeVersion: () => ({ mutate: jest.fn(), isPending: false }),
  }),
);
jest.mock(
...
import { Route } from "src/app/routes/admin/settings/consent/document-types/$typeId/versions/$versionId";
...
const draftVersion = {
  ...
  status: "draft",
  translations: [
    { locale: "en", name: "Privacy Agreement", description: "Privacy data consent" },
    { locale: "fr", name: "Accord de confidentialité", description: "Consentement vie privée" },
  ],
};
...
describe("Admin Document Type Version Detail Route", () => {
  beforeEach(() => {
    mockRouteUseParams.mockReturnValue({ typeId: "dtype-001", versionId: "dtv-001" });
  ...
  describe("Route registration", () => {
    it("Should register at /admin/settings/consent/document-types/$typeId/versions/$versionId", () => {
  ...
  describe("Loader", () => {
    it("Should call ensureQueryData once for the version", async () => {
      mockEnsureQueryData.mockResolvedValue({ version: 1, id: "dtv-001" });
      await typedRoute.options.loader({
        params: { typeId: "dtype-001", versionId: "dtv-001" },
      });
  ...
  describe("Breadcrumbs", () => {
    it("Should return static breadcrumbs with Settings, Document Types, and Version", () => {
  ...
  describe("Error state", () => {
    it("Should show error message", () => {
      mockUseQuery.mockReturnValue({
        data: null,
        isLoading: false,
        error: { message: "Document type version not found" },
      });
      render(<typedRoute.options.component />);
      expect(screen.getByText("Error: Document type version not found")).toBeInTheDocument();
    });
  });
