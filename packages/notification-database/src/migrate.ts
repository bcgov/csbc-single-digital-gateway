...
// works locally regardless of cwd. In CI/containers NOTIFICATION_DATABASE_URL comes from the
// environment (dotenv never overrides an already-set var, and a missing .env is a no-op).
...
async function run(): Promise<void> {
  ...
  if (!url) {
    throw new Error('db:migrate — NOTIFICATION_DATABASE_URL is not set');
  }

  const db = createDatabase(url, {
    ssl: resolvePgSsl({
      mode: process.env.PGSSLMODE,
      ca: process.env.NOTIFICATION_DATABASE_CA_CERT,
    }),
  });
  try {
